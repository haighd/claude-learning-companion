#!/usr/bin/env python3
"""
CLC Pre-Tool Hook - Auto-inject relevant heuristics and complexity warnings.

This hook closes the learning loop by:
1. Auto-querying CLC for relevant heuristics
...
Works with: Task tool (subagent spawning)
"""
...
def format_learning_context(
    ...
    """Format the learning context for injection into agent prompt."""
    lines = [
        "",
        "---",
        "## CLC Knowledge (Auto-Injected)",
        ""
    ]

    # Complexity warning (if applicable)
    if complexity and complexity['level'] in ('HIGH', 'MEDIUM'):
        warning_symbol = "!!" if complexity['level'] == 'HIGH' else "!"
