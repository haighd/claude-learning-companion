...
_STRUCTURE_PATTERN = re.compile(r'(?m)^#{1,3}\s+\w+')
...
def determine_outcome(tool_output: dict) -> Tuple[str, str]:
    ...
    for compiled_pattern, reason in _SUCCESS_PATTERNS:
    ...
    for compiled_pattern, reason in _FAILURE_PATTERNS:
        ...
        if match:
            ...
            is_false_positive = any(fp.search(context) for fp in _FALSE_POSITIVE_PATTERNS)
