...
Exposes the Claude Learning Companion to external tools via Model Context Protocol.
...
```bash
claude mcp add CLC_FLOW_MCP python3.13 ~/.claude/clc/mcp/clc_server.py
```
...
```json
{
  "mcpServers": {
    "CLC_FLOW_MCP": {
      "command": "python3.13",
      "args": ["~/.claude/clc/mcp/clc_server.py"]
    }
  }
}
```
...
1. Claude-flow pre-task hook: `clc_query(depth="standard")`
2. Claude-flow post-task hook: `clc_record_heuristic()` for lessons learned
3. Golden rules from CLC are respected in all operations
4. CLC = "brain" (knowledge), Claude-flow = "nervous system" (orchestration)
